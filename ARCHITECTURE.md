...
El sistema pasó por 3 iteraciones ADD (base + seguridad → pipeline IoT → pagos seguros) y luego una serie de mejoras de auditoría/observabilidad no cubiertas por una iteración ADD formal: split de MySQL a un contenedor por servicio, Redis para blacklist distribuido, RabbitMQ para el Outbox, y OpenTelemetry/Jaeger para tracing. Ver [`microservices/docs/migrations/support-migrations.md`](microservices/docs/migrations/support-migrations.md) para el detalle de esos cambios de soporte.
