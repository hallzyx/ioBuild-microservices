...
El sistema pasó por 4 iteraciones ADD: base + seguridad (QA-1) → pipeline IoT (QA-2) → pagos seguros (QA-3) → observabilidad y tracing distribuido (QA-4). Además hubo mejoras de auditoría que no responden a ningún driver de esas 4 iteraciones: split de MySQL a un contenedor por servicio y la extensión de RabbitMQ como bus de eventos entre Devices/Projects/IAM. Ver [`microservices/docs/migrations/support-migrations.md`](microservices/docs/migrations/support-migrations.md) para el detalle de esos cambios de soporte, y [`microservices/docs/iterations/iteration-4-observabilidad.md`](microservices/docs/iterations/iteration-4-observabilidad.md) para el detalle de la Iteración 4.
