...
/// <summary>
/// Background service that polls the Devices outbox table and publishes pending events
...
///   so it shares no DbContext with the singleton publisher.
/// - <see cref="RunOneCycleAsync"/> is <c>internal</c> virtual for unit-testing without hosting.
/// </summary>
public class OutboxWorker : BackgroundService
{
    ...
    {
        _logger.LogInformation("Devices OutboxWorker starting. Poll interval: {PollMs}ms", _pollIntervalMs);
    ...
    public virtual async Task RunOneCycleAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Devices OutboxWorker: failed to query pending messages");
            return;
        }
        ...
        foreach (var msg in pending)
        {
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Devices OutboxWorker: unknown EventType={EventType} for OutboxMessage id={Id}. Skipping.",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "Devices OutboxWorker: publish failed for {EventType} id={Id}. RetryCount={Retry}",
            ...
            {
                _logger.LogError(ex, "Devices OutboxWorker: failed to update OutboxMessage id={Id}", msg.Id);
            }
        }
    }
