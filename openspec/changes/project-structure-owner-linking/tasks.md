...
- [ ] 3.1 [RED] Write `UnitAggregateTests`: `ComposeUnitNumber(5,"02") == "5-02"`; `OwnerId` is null on construction; `LinkOwner(42)` sets `OwnerId=42`; `AssignOwnerEmail` lower-cases the value.
- [ ] 3.2 [GREEN] Update `IoBuild.Projects/Domain/Model/Aggregates/Unit.cs` (§1.1): add `Floor`, `RoomNumber`, `OwnerEmail`, change `OwnerId` to `int?`; add `LinkOwner`, `AssignOwnerEmail`, `ComposeUnitNumber`; keep legacy ctor with `Floor=0, RoomNumber=unitNumber`.
- [ ] 3.3 [GREEN] Add `IoBuild.Projects/Domain/Model/Entities/RegisteredOwner.cs` (§3.3): `Email` (PK, lower-cased), `UserId`, `LastEventAt`.
- [ ] 3.4 [GREEN] Update `IoBuild.Projects/Infrastructure/Persistence/EFC/AppDbContext.cs` (§1.4): extend `Unit` EF config (new columns, nullable `OwnerId`, unique index `(ProjectId,Floor,RoomNumber)`, index on `OwnerEmail`); add `RegisteredOwner` block (`HasKey(Email)`, maxlen 255, table `registered_owner`).
- [ ] 3.5 [GREEN] Update `IoBuild.Projects/Infrastructure/Persistence/ProjectsSeedData.cs` (§1.6): add explicit `Floor`, `RoomNumber` values to all 5 existing seed units; set `OwnerEmail=null`; keep existing `OwnerId` values.
- [ ] 3.6 Run `dotnet ef migrations add AddUnitStructureAndOwnerLinking -p IoBuild.Projects`; verify generated migration matches §1.5 (addColumn `floor`, `room_number`, `owner_email`; alterColumn `owner_id` nullable; create `registered_owner` table; add indexes).
- [ ] 3.7 [RED] Write `DefineProjectStructureCommandTests` (uses `ProjectsDbFixture`): (a) happy path creates `floors × unitsPerFloor` units with correct `Floor`/`RoomNumber`; (b) `UnitId > 0` in each outbox `UnitCreatedEvent`; (c) one `FloorStructureDefinedEvent` outbox row per floor; (d) HTTP 409 when project already has units (REQ-PS-03); (e) 422 on `floors < 1`; (f) unit-first owner match: if `RegisteredOwner` row exists for the assigned email, `OwnerId` is set immediately and `UnitOwnerMatchedEvent` outbox row is written.
- [ ] 3.8 [GREEN] Add `IoBuild.Projects/Domain/Services/Commands/Projects/DefineProjectStructureCommand.cs`: records `RoomSpec`, `FloorSpec`, `DefineProjectStructureCommand` (§1.3).
...
- [ ] 3.10 [GREEN] Add `POST /api/v1/projects/{id}/structure` endpoint in `IoBuild.Projects/Interfaces/REST/ProjectsController.cs`; Builder-role JWT guard; map request → `DefineProjectStructureCommand`.
- [ ] 3.11 Verify all scenario tests PS-S01…PS-S07 are green; `dotnet test IoBuild.Projects.Tests`.
