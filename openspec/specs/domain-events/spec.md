...
- `DateTime OccurredOn` — UTC timestamp set at event creation. (Note: code uses `OccurredOn`; spec aligned to implementation.)
- A typed payload with the resource state at the time of occurrence.

Events defined in this capability:
...
And the event EventId is a non-empty Guid
...
```
Given an outbox row for DeviceCreated (EventId = E1, DeviceId = X) was published successfully
...
Then exactly one outbox row for DeviceDeletedEvent is written with DeviceId = X
...
Then exactly one outbox row for ProjectCreatedEvent is written
...
Then exactly one outbox row for UnitCreatedEvent is written
And the OutboxWorker publishes it to RabbitMQ
```
